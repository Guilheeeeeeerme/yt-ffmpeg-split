...
RUN echo "    ffmpeg -ss \$1 -t \$2 -i input.mp3 fragment-\$1.mp3"  >> out.sh
RUN echo '}'  >> out.sh
...
# RUN sed -i 's/\[silencedetect.*/ffmpeg -ss <\$silence_end - 0.25> -t <\$silence_duration + 2 * 0.25> -i input.mp3 fragment-N.mp3/g' out.sh 
# RUN sed -i 's/\[silencedetect.*/ffmpeg -ss \$silence_end -t \$silence_duration -i input.mp3 fragment-\$silence_end.mp3 | true/g' out.sh 
# RUN echo "ffmpeg -ss \$silence_end -t \$silence_duration -i input.mp3 fragment-\$silence_end.mp3" >> out.sh
...
RUN echo "mv fragment-* data/" >> out.sh
